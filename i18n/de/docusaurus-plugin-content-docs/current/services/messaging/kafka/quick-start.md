...
Dieser Leitfaden begleitet Sie pas à pas dans le Deployment de votre premier **cluster Kafka** sur Hikube, du manifeste YAML jusqu'aux premiers tests de messagerie.
...
- **3 brokers Kafka** et **3 nœuds ZooKeeper** pour la Hochverfügbarkeit
...
Erstellen Sie eine Datei `kafka.yaml` avec la configuration suivante :
...
:::tip
Kafka ne dispose pas d'authentification par défaut sur Hikube. Pour un usage en production, il est recommandé de ne pas exposer le cluster à l'extérieur (`external: false`). Consultez la [API-Referenz](./api-reference.md) pour la configuration complète.
:::
...
Kafka auf Hikube ne dispose pas d'authentification par défaut. Les connexions se font directement via le service bootstrap :
...
- **[API-Referenz](./api-reference.md)** : Configuration complète de toutes les options Kafka
- **[Übersicht](./overview.md)** : Architecture détaillée et cas d'usage Kafka auf Hikube
