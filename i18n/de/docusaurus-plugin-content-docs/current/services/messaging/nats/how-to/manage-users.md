---
title: "Verwaltung von les utilisateurs"
---

# Verwaltung von les utilisateurs NATS

Dieser Leitfaden erklärt comment créer et gérer les utilisateurs d'un cluster NATS auf Hikube de manière déclarative via les manifestes Kubernetes.
...
:::warning
Si `external: true` est aktiviert, le cluster NATS est accessible depuis l'extérieur du cluster Kubernetes. Assurez-vous que tous les utilisateurs disposent de mots de passe robustes.
:::

### 4. Vérifier les connexions aktivierts

Vous pouvez vérifier les connexions aktivierts sur le cluster NATS :
...
- Les pods NATS sont tous en état `Running` après la Aktualisierung
- Chaque utilisateur peut se connecter avec son mot de passe
- Un mot de passe incorrect est rejeté (`Authorization Violation`)
- Les connexions aktivierts sont visibles dans le rapport du serveur

## Weiterführende Informationen

- **[API-Referenz NATS](../api-reference.md)** : documentation complète des paramètres `users`
- **[Konfiguration von JetStream](./configure-jetstream.md)** : aktiviertr la persistance des messages et le streaming
