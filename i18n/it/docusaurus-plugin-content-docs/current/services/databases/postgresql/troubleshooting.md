...
**Causa** : le PersistentVolumeClaim (PVC) ne parvient pas à se lier à un volume. Cela peut être dû à une `storageClass` inexistante, un quota de stockage dépassé ou un manque de ressources sur les nœuds.
...
**Causa** : un décalage (lag) de réplication peut survenir en raison d'une charge réseau élevée, de ressources insuffisantes sur les standby, ou d'un volume de transactions important sur le primary.
...
**Causa** : les pods ne sont pas en cours d'exécution, le nom du Secret est incorrect, ou le service n'est pas accessible.
...
**Causa** : les paramètres de bootstrap sont mal configurés. Le champ `bootstrap.oldName` doit correspondre exactement au nom de l'instance d'origine, et le nom de la nouvelle instance doit être différent.
...
**Causa** : les paramètres PostgreSQL ne sont pas adaptés à la charge de travail, ou les ressources allouées sont insuffisantes.
